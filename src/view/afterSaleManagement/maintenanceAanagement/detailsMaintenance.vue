<template>
    <div class="refundReturnInfo">
        <h3>维修:{{this.refundForm.process_status_name}}</h3>
        <h3>维修信息:</h3>
        <div style="padding-left: 40px">
            <p v-if="this.refundForm.refund_reason"> 维修原因:<span>{{this.refundForm.refund_reason}}</span></p>
            <p v-if="this.refundForm.rrefund_amount">维修金额:<span>{{this.refundForm.refund_amount}}</span></p>
            <p v-if="this.refundForm.refund_des">维修说明:<span>{{this.refundForm.refund_des}}</span></p>
            <p v-if="this.refundForm.pictures!==null">退货退款凭证:<img :src=this.refundForm.pictures[0] alt="" v-if="this.refundForm.refund_pictures!==null" style="width: 80px;height:80px;vertical-align: middle"></p>
...
<script>
    ...
    export default {
        ...
        methods:{
            getInfo(){
                this.$getApi(`/shop/repairs/goods/${this.$route.query.row.id}`).then(res=>{
                    console.log(res,"res")
                    this.refundForm=res.data.data
                    this.tableData.push(this.refundForm)
                    this.tableData.map(item=>{
                        item.good_name=item.repair.good_name
                        item.piceure=item.repair.good_info.images[0]
                        item.sku_amount=item.repair.sku_amount
                        item.sku_real_amount=item.repair.sku_real_amount
                    })
